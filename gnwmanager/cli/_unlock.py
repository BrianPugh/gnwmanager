...
def unlock(
    ...
    *,
    gnw: GnWType,
    no_backup: Annotated[bool, Parameter(negative="")] = False,
):
    ...
    no_backup: bool
        Do NOT perform any backup operations; this WILL erase both internal and external flash.
        If you do not have a previous backup, you will be unable to get back the original firmware
        for dual boot or to restore the device to factory condition.
    """
    if isinstance(gnw.backend, PyOCDBackend):
        raise TypeError("Device unlocking requires using --backend=openocd")

    if no_backup:
        print("This will completely erase your device!")
        print("Only continue if you have a complete backup, or you don't care about losing original firmware.")
        ...
        print("Unlocking complete! Completely power cycle your device (remove battery & usb power).")
    else:
